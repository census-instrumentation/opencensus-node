...
function assertSpanAttributes(
  ...
  assert.strictEqual(
    span.status.code,
    HttpsPlugin.parseResponseStatus(httpStatusCode)
  );
  assert.strictEqual(
    span.attributes[HttpsPlugin.ATTRIBUTE_HTTP_HOST],
    hostName
  );
  assert.strictEqual(
    span.attributes[HttpsPlugin.ATTRIBUTE_HTTP_METHOD],
    httpMethod
  );
  ...
  if (userAgent) {
    assert.strictEqual(
      span.attributes[HttpsPlugin.ATTRIBUTE_HTTP_USER_AGENT],
      userAgent
    );
  }
  assert.strictEqual(
