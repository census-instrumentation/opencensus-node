...
import {
  StatsViewData,
  StatszParams,
} from '../src/zpages-frontend/page-handlers/statsz.page-handler';
import {
  TraceConfigzData,
  TraceConfigzParams,
} from '../src/zpages-frontend/page-handlers/traceconfigz.page-handler';
import {
  TracezData,
  TracezParams,
} from '../src/zpages-frontend/page-handlers/tracez.page-handler';
...
class ZpagesJSONClient {
  ...
  private async getEndpoint<P, R>(
    endpoint: string,
    query?: Partial<P>
  ): Promise<R> {
    query = Object.assign({ json: '1' }, query);
    const response = await axios.get(
      `${this.origin}/${endpoint}?${qs.stringify(query)}`
    );
   ...
   * @param query Optional query parameters.
   */
  async getTraceConfigz(
    query?: Partial<TraceConfigzParams>
...
describe('Zpages Exporter', () => {
  ...
  describe('running Zpages Server', () => {
    ...
    it('should access tracez page', done => {
      http
        .get(zpagesServerUrl + '/tracez', res => {
    ...
    it('should access trace config page', done => {
      http
        .get(zpagesServerUrl + '/traceconfigz', res => {
          assert.strictEqual(res.statusCode, 200);
          done();
        })
        .on('error', done);
    ...
    it('should access statsz page', done => {
      http
        .get(zpagesServerUrl + '/statsz', res => {
          assert.strictEqual(res.statusCode, 200);
          done();
        })
        .on('error', done);
  ...
  describe('when a view is accessed in statsz page', () => {
    ...
    describe('with LAST VALUE aggregation type', () => {
      it('should get view information', async () => {
        ...
        zpagesData = await zpagesClient.getStatsz({
          path: 'test/LastValueView',
        });
