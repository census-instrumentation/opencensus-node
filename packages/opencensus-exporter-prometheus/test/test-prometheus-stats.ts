...
describe('Prometheus Stats Exporter', () => {
    ...
    http
      .get(prometheusServerUrl, res => {
        res.on('data', chunk => {
          ...
          assert.strictEqual(
            lines[1],
            '# TYPE ocnodemetrics_countview counter'
          );
          assert.strictEqual(
            lines[2],
            'ocnodemetrics_countview{tagKey1="tagValue1"} 2'
          );
          done();
        });
      })
  ...
  it('should create a last value aggregation', done => {
    ...
    const view = globalStats.createView(
      'ocnodemetrics/lastvalueview',
      measure,
      AggregationType.LAST_VALUE,
      tagKeys,
      'A last value aggregation example'
    );
    const measurement = { measure, value: 2 };
    const measurement2 = { measure, value: 3 };
    globalStats.registerView(view);
    ...
    http
      .get(prometheusServerUrl, res => {
        res.on('data', chunk => {
          ...
          assert.strictEqual(
            lines[1],
            '# TYPE ocnodemetrics_lastvalueview gauge'
          );
          assert.strictEqual(
            lines[2],
  ...
  it('should create a distribution aggregation', done => {
    ...
    const view = globalStats.createView(
      'ocnodemetrics/distributionview',
      measure,
      AggregationType.DISTRIBUTION,
      tagKeys,
      'A distribution aggregation example',
      boundaries
    );
    ...
    http
      .get(prometheusServerUrl, res => {
        res.on('data', chunk => {
          ...
          assert.strictEqual(
            lines[1],
            '# TYPE ocnodemetrics_distributionview histogram'
          );
          assert.strictEqual(
            lines[2],
  ...
  it('should throw error when labels contains "le" label name in histogram label names', () => {
    ...
    const boundaries = [10, 20, 30, 40];
    const view = globalStats.createView(
      'ocnodemetrics/distributionview1',
      measure,
      AggregationType.DISTRIBUTION,
      tagKeys,
      'A distribution aggregation example',
    ...
    http
      .get(prometheusServerUrl, res => {
        res.on('data', chunk => {
          ...
          assert.strictEqual(lines[1], '# TYPE opencensus_test_key_1 counter');
          assert.strictEqual(
            lines[2],
            'opencensus_test_key_1{le="tagValue1"} 2'
          );
          done();
        });
      })
