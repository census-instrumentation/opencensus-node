...
export class SimpleModulePlugin extends classes.BasePlugin {
  ...
  applyPatch(moduleExporters: any, tracer: types.Tracer, version: string) {
    ...
    shimmer.wrap(
        moduleExporters, 'name', (orig) => () => 'patched-' + orig.apply());

    shimmer.wrap(moduleExporters, 'value', (orig) => () => orig.apply() + 1);
    return moduleExporters;
  }

  applyUnpatch(): void {
    shimmer.unwrap(this.moduleExporters, 'name');
    shimmer.unwrap(this.moduleExporters, 'value');
  }
}
